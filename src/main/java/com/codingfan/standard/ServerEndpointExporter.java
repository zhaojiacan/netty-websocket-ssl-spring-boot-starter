...
public class ServerEndpointExporter extends ApplicationObjectSupport implements SmartInitializingSingleton {
    ...
    private ServerEndpointConfig buildConfig(ServerEndpoint annotation) {
        ...
        boolean sslEnable = annotation.sslEnable();
        String sslType = annotation.sslType();
        String sslKeyStorePath = annotation.sslKeyStorePath();
        String sslKeyStorePassword = annotation.sslKeyStorePassword();
        ...
        if (!StringUtils.isEmpty(prefix)) {
            ...
            Boolean sslEnableEnv = environment.getProperty(prefix + ".ssl.enable", Boolean.class);
            if (sslEnableEnv != null) {
                sslEnable = sslEnableEnv;
            }
            String sslTypeEnv = environment.getProperty(prefix + ".ssl.type", String.class);
            if (sslTypeEnv != null && sslTypeEnv!="") {
                sslType = sslTypeEnv;
            }
            String sslKeyStorePathEnv = environment.getProperty(prefix + ".ssl.key-store.path", String.class);
            if (sslKeyStorePathEnv != null && sslKeyStorePathEnv!="") {
                sslKeyStorePath = sslKeyStorePathEnv;
            }
            String sslKeyStorePasswordEnv = environment.getProperty(prefix + ".ssl.key-store.password", String.class);
            if (sslKeyStorePasswordEnv != null && sslKeyStorePasswordEnv!="") {
                sslKeyStorePassword = sslKeyStorePasswordEnv;
            }
        }

        ServerEndpointConfig serverEndpointConfig = new ServerEndpointConfig(host, port, path, bossLoopGroupThreads, workerLoopGroupThreads, useCompressionHandler, optionConnectTimeoutMillis, optionSoBacklog, childOptionWriteSpinCount, childOptionWriteBufferHighWaterMark, childOptionWriteBufferLowWaterMark, childOptionSoRcvbuf, childOptionSoSndbuf, childOptionTcpNodelay, childOptionSoKeepalive, childOptionSoLinger, childOptionAllowHalfClosure, readerIdleTimeSeconds, writerIdleTimeSeconds, allIdleTimeSeconds,maxFramePayloadLength,sslEnable,sslType,sslKeyStorePath,sslKeyStorePassword);
